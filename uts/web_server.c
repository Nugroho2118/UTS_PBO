    ...
    char *post_data)
{
    ...
    char fileURL[256];
    snprintf(
        fileURL, sizeof(fileURL), "%s%s", FOLDER_DOCUMENT, uri);
    FILE *file = fopen(fileURL, "rb");

    // Jika file resource tidak ditemukan, // maka kirimkan status 404 ke browser
    if (!file)
    {
...
void handle_client(int sock_client)
{
    ...
    parse_request_line(
        request, method, uri, http_version, query_string, post_data);
    handle_method(
        &response, &response_size, method, uri, http_version, query_string, post_data);
    printf("\n-----------------------------------------------\n");
...
int main()
{
    ...
    if (setsockopt(
            sock_server, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)))
    {
    ...
    if (bind(
            sock_server, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
    {
    ...
    while (1)
    {
        if ((sock_client = accept(
                 sock_server, (struct sockaddr *)&serv_addr, (socklen_t *)&addrlen)) < 0)
        {
